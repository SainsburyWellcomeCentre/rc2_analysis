...
threshold       = 1.5;   % 2nd-pass relaxed further (first pass 2.5, prior 2nd-pass 2.0)
...
% Stimulus file path
% -------------------------------------------------------------------------
goggles_stim_fname = '/Users/lauraporta/local_data/motion_clouds/visual_stimuli/sparse_noise/sparse_noise_wisecoco_2500stims.mat';

% -------------------------------------------------------------------------
% Output directories
% -------------------------------------------------------------------------
goggles_combined_dir = '/Users/lauraporta/local_data/motion_clouds/saved_goggles/_rfs/2ndpass';
goggles_comparison_dir = '/Users/lauraporta/local_data/motion_clouds/saved_goggles/_rfs/2ndpass';
...
for probe_i = 1 : length(probe_ids)
    ...
    % --- 2nd-pass: restrict to the V-responsive no-RF target clusters only ---
    % (first-pass 2.5-sigma RFs for the rest are kept; these get a relaxed 2.0-sigma
    %  remap, flagged separately as second-pass).
    tmap = containers.Map( ...
        {'CAA-1124370_rec1_rec2_rec3', 'CAA-1124371_rec1_rec2_rec3'}, ...
        {[179, 203, 246, 265, 422, 659, 239], [4, 26, 40, 50, 52, 62, 60]});
    target_ids = tmap(this_probe{1});
    cids = arrayfun(@(c) c.id, clusters);
    clusters = clusters(ismember(cids, target_ids));
    fprintf('  2nd-pass: restricted to %d target clusters: %s\n', ...
        numel(clusters), mat2str(target_ids));
    ...
    csv_data.cluster_id = [];
    csv_data.region = {};
    csv_data.rf_type = {};  % 'white', 'black', or 'none' (no blob at this threshold)
    csv_data.source = {};   % '2ndpass' (measured blob) or 'nofit' (no blob -> impute downstream)
    ...
    for i = 1 : size(sta_white_combined_store, 3)
        ...
        % Track whether this cluster produces ANY measured RF row; if not, a
        % 'nofit' row is appended at the end so the layer-average fallback
        % (same probe + same VISp layer) can impute its centre downstream.
        n_rows_before = numel(csv_data.cluster_id);
        ...
        if any(RF_comb(:))
            ...
            if ~isempty(props_black)
                % OFF RFs are often split into two physiologically-valid regions.
                % Per Laura's convention, take the centre BETWEEN the regions:
                % the mean of all component centroids (unweighted midpoint). For a
                % single component this is just that component's centroid.
                cents_black = cat(1, props_black.Centroid);   % n x 2 [x, y] grid squares
                centroid_black = mean(cents_black, 1);
        ...
        % No measured blob (white or black) at this threshold -> emit a 'nofit'
        % row carrying the cluster's region so the downstream layer-average
        % fallback can impute a centre. NaN centroids are skipped by the Gabor
        % extraction's float() parse, so this row is inert until imputed.
        if numel(csv_data.cluster_id) == n_rows_before
            csv_data.cluster_id(end+1) = c.id;
            csv_data.region{end+1} = c.region_str;
            csv_data.rf_type{end+1} = 'none';
            csv_data.source{end+1} = 'nofit';
            csv_data.centroid_azimuth_pixels(end+1) = NaN;
            csv_data.centroid_elevation_pixels(end+1) = NaN;
            csv_data.centroid_azimuth_deg(end+1) = NaN;
            csv_data.centroid_elevation_deg(end+1) = NaN;
            csv_data.size_azimuth_pixels(end+1) = NaN;
            csv_data.size_elevation_pixels(end+1) = NaN;
            csv_data.size_azimuth_deg(end+1) = NaN;
            csv_data.size_elevation_deg(end+1) = NaN;
        end
    ...
    try, join_pdfs(combined_fnames, new_fname_combined, true); catch jpe, warning("join_pdfs failed: %s", jpe.message); end    
    % =====================================================================
    ...
    if ~isempty(csv_data.cluster_id)
        ...
        rf_table = table(csv_data.cluster_id', csv_data.region', csv_data.rf_type', ...
                         ...
                         'VariableNames', {'cluster_id', 'region', 'rf_type', 'source', ...
        ...
        csv_fname = fullfile(goggles_combined_dir, ...
            sprintf('%s_rf_metrics_2ndpass.csv', probe_ids{probe_i}));
    ...
    if show_individual_sessions
        ...
        try
            ...
            warning('join_pdfs failed: %s', jpe.message);
        end
    end
