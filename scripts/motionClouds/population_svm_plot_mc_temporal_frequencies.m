% population_svm_plot_mc_temporal_frequencies.m
% Read single-cluster temporal frequencies CSV and produce population plots/CSVs
% Parts:
% 1) clusters responsive in 'V' with temporal-frequency tuning (at least one response_direction==1)
% 2) clusters responsive in 'V' but no temporal-frequency tuning (all response_direction==0 and wilcoxon<0.05)
...
% Part 1: clusters with in 'V' at least one temporal-frequency with response_direction == 1
...
part1_clusters = find(part1_idx);
    ...
    for ii = 1:numel(part1_clusters)
        ...
        % pick temporal-frequency with highest median (ignore NaNs)
    ...
    % Label only the preferred-temporal-frequency position (x=2) as V_pref_ori
    labs = repmat({''}, 1, numel(prefixes_all));
    labs{2} = 'V_pref_ori';
    xticklabels(labs);
    xlabel('Temporal frequency (rotated so pref at x=2)'); ylabel('Median(Response - Baseline) (Hz)');
    ...
    % save figure
    pdfname = fullfile(out_dir, 'population_temporal_part1_V_tuned.pdf');
    ...
    if ~isempty(summary_rows)
        ...
        writetable(S, fullfile(out_dir,'population_temporal_part1_summary.csv'));
    end
else
    fprintf('Part1: no clusters found\n');
end

% Part 2: clusters responsive in V, but no temporal-frequency tuning
...
part2_clusters = find(part2_idx);
    ...
    hV = plot(x, medianV, '-o', 'Color', darkV, 'MarkerFaceColor', darkV, 'LineWidth',1.5);
    hVT = plot(x, medianVT, '-o', 'Color', darkVT, 'MarkerFaceColor', darkVT, 'LineWidth',1.5);
    title(sprintf('Part2: V responsive but no temporal-frequency tuning (n=%d out of %d)', row_ctr, nClusters));
    xticks(1:numel(prefixes_all)); xticklabels(prefixes_all);
    xlabel('Temporal frequency'); ylabel('Median(Response - Baseline) (Hz)'); legend([hV,hVT],{'V (median)','VT (median)'});
    pdfname = fullfile(out_dir, 'population_temporal_part2_V_responsive_no_tuning.pdf');
    set(fig,'PaperPositionMode','auto'); print(fig, pdfname, '-dpdf'); close(fig);
    if ~isempty(summary_rows)
        S = struct2table([summary_rows{:}]);
        writetable(S, fullfile(out_dir,'population_temporal_part2_summary.csv'));
    end
else
...
part3_clusters = find(part3_idx);
    ...
    xticks(1:numel(prefixes_all));
    % Label only the preferred-temporal-frequency position (x=2) as VT_pref_ori
    labs = repmat({''}, 1, numel(prefixes_all));
    labs{2} = 'VT_pref_ori';
    xticklabels(labs);
    xlabel('Temporal frequency (rotated so VT pref at x=2)'); ylabel('Median(Response - Baseline) (Hz)'); legend([hVT,hV],{'VT (median)','V (median)'});
    pdfname = fullfile(out_dir, 'population_temporal_part3_V_not_responsive_VT_tuned.pdf');
    set(fig,'PaperPositionMode','auto'); print(fig, pdfname, '-dpdf'); close(fig);
    if ~isempty(summary_rows)
        S = struct2table([summary_rows{:}]);
        writetable(S, fullfile(out_dir,'population_temporal_part3_summary.csv'));
    end
else
    fprintf('Part3: no clusters found\n');
end

% Part 4: V not responsive, VT responsive but not temporal-frequency tuned
...
for i=1:nClusters
    ...
    allOnesVT = all(dVT == 1);
    % Part4: (V all zeros & wV>0.05) AND ((VT all zeros & wWT<0.05 && medVT_overall>0) OR (all VT ones & kruskal>0.05))
    if allZerosV && (~isnan(wV) && wV > 0.05)
...
part4_clusters = find(part4_idx);
    ...
    xticks(1:numel(prefixes_all)); xticklabels(prefixes_all);
    xlabel('Temporal frequency'); ylabel('Median(Response - Baseline) (Hz)'); legend([hVT,hV],{'VT (median)','V (median)'});
    pdfname = fullfile(out_dir, 'population_temporal_part4_VT_responsive_no_tuning.pdf');
    set(fig,'PaperPositionMode','auto'); print(fig, pdfname, '-dpdf'); close(fig);
    if ~isempty(summary_rows)
        S = struct2table([summary_rows{:}]);
        writetable(S, fullfile(out_dir,'population_temporal_part4_summary.csv'));
    end
else
...
if ~isempty(multi_idx)
    ...
    try
        ...
        writetable(Tdiag, fullfile(out_dir,'cluster_parts_diagnostic_temporal.csv'));
    catch
        warning('Could not write diagnostic CSV for temporal');
    end
else
