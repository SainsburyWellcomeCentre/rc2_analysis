% population_summary_mc_orientations.m
...
for i = 1:nClusters
    ...
    for g = 1:length(trial_groups)
        ...
        if any(sel_all)
            ...
            % try both orientation/temporal/spatial kruskal names if present
...
% (classify_group moved to end of file to satisfy MATLAB script function ordering)
...
for gi = 1:2
    ...
    out_csv = fullfile(out_dir, sprintf('population_orientation_summary_%s_categories.csv', grp));
    writetable(Tcats, out_csv);
    % plot categories as percentage bar
    fig = figure('Visible','off');
    bar(categorical(cats), pct);
    ylabel('Percentage of clusters');
    title(sprintf('Population categories (%s) - orientations', grp));
    ylim([0 100]);
    pdfname = fullfile(out_dir, sprintf('population_orientation_summary_%s_categories.pdf', grp));
    set(fig,'PaperPositionMode','auto'); print(fig, pdfname, '-dpdf'); close(fig);

    % preferred orientation distribution for tuned clusters
    ...
    if sum(pref_cnts) > 0
        ...
        out_csv2 = fullfile(out_dir, sprintf('population_orientation_summary_%s_pref_distribution.csv', grp));
        writetable(Tpref, out_csv2);
        fig2 = figure('Visible','off');
        bar(categorical(prefixes_all), pref_pct);
        ylabel('Percent of tuned clusters');
        xlabel('Prefix');
        title(sprintf('Preferred prefix distribution (%s) - tuned clusters', grp));
        ylim([0 100]);
        pdfname2 = fullfile(out_dir, sprintf('population_orientation_summary_%s_pref_distribution.pdf', grp));
...
fprintf('Population orientation summaries complete. Files written to: %s\n', out_dir);
