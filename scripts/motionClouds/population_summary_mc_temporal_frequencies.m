% population_summary_mc_temporal_frequencies.m
% Summarise population classification for temporal frequencies (V and VT)
...
input_csv = fullfile('C:','Users','lee','Documents','mvelez','figures','motionClouds', ...
    'passive_same_luminance_mc','stationary_vs_motion_temporal_frequencies','single_cluster', ...
    'motion_cloud_temporal_frequencies_analysis_summary.csv');
...
out_dir = fullfile('C:','Users','lee','Documents','mvelez','figures','motionClouds', ...
    'passive_same_luminance_mc','stationary_vs_motion_temporal_frequencies','summary');
...
for gi = 1:2
    ...
    out_csv = fullfile(out_dir, sprintf('population_temporal_summary_%s_categories.csv', grp));
    writetable(Tcats, out_csv);
    % plot categories as percentage bar
    fig = figure('Visible','off');
    bar(categorical(cats), pct);
    ylabel('Percentage of clusters');
    title(sprintf('Population categories (%s) - temporal frequencies', grp));
    ylim([0 100]);
    pdfname = fullfile(out_dir, sprintf('population_temporal_summary_%s_categories.pdf', grp));
    ...
    if sum(pref_cnts) > 0
        ...
        out_csv2 = fullfile(out_dir, sprintf('population_temporal_summary_%s_pref_distribution.csv', grp));
        writetable(Tpref, out_csv2);
        fig2 = figure('Visible','off');
        bar(categorical(prefixes_all), pref_pct);
        ylabel('Percent of tuned clusters');
        xlabel('Prefix');
        title(sprintf('Preferred prefix distribution (%s) - tuned clusters (temporal)', grp));
        ylim([0 100]);
        pdfname2 = fullfile(out_dir, sprintf('population_temporal_summary_%s_pref_distribution.pdf', grp));
...
fprintf('Population temporal summaries complete. Files written to: %s\n', out_dir);
