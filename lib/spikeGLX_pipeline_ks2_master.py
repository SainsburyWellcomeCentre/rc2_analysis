...
logName = 'CAA-1115211_pairs_log.csv'

# Raw data directory = npx_directory
# run_specs = name, gate, trigger and probes to process
npx_directory = r'D:\mvelez\mateoData_probe\ecephys_spike_sorting_test\kilosort2_cuda11\CAA-1115211_pairs'
...
run_specs = [['CAA-1115211_pairs', '0', '0,0', '0']]
...
catGT_dest = r'D:\mvelez\mateoData_probe\ecephys_spike_sorting_test\kilosort2_cuda11\CAA-1115211_pairs\output'
...
json_directory = r'D:\mvelez\mateoData_probe\ecephys_spike_sorting_test\kilosort2_cuda11\CAA-1115211_pairs\json_files'
...
for spec in run_specs:
    ...
    first_trig, last_trig = SpikeGLX_utils.ParseTrigStr(spec[2], spec[3], spec[1], prb0_fld)
    trigger_str = repr(first_trig) + ',' + repr(last_trig)

    fileName = run_folder_name + '_t' + repr(first_trig) + '.imec' + prb_list[0] + '.ap.bin'
    continuous_file = os.path.join(prb0_fld, fileName)
    metaName = run_folder_name + '_t' + repr(first_trig) + '.imec' + prb_list[0] + '.ap.meta'
    input_meta_fullpath = os.path.join(prb0_fld, metaName)
    ...
    info = createInputJson(input_json, npx_directory=npx_directory, 
                                       ...
                                       ks_minfr_goodchannels = 0
                                       )

    # CatGT operates on whole runs with multiple probes, so gets called in just
    # once per run_spec
    if run_CatGT:
        command = sys.executable + " -W ignore -m ecephys_spike_sorting.modules." + 'catGT_helper' + " --input_json " + input_json \
    ...
    for i, prb in enumerate(prb_list):
        ...
        fileName2 = run_str + '_tcat.imec' + prb + '.ap.meta'
        input_meta_path = os.path.join(data_directory, fileName2)
