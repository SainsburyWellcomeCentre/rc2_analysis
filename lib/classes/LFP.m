classdef LFP < handle
    ...
    methods
        ...
        function obj = LFP(glx_dir)
        %% LFP()
        %   Class controlling access to LFP data and computation of
        %   properties.
        %       glx_dir = directory with SpikeGLX lf.bin and lf.meta data
        
            % find the lf.bin data
            dir_contents = dir(glx_dir);
            
            % find entry with the lf.bin file
            lf_idx = contains({dir_contents(:).name}, '.lf.bin');
            
            % make sure there is one and only one file
            assert(sum(lf_idx) == 1, 'more or less than 1 .lf.bin file found.');
            ...
            obj.meta_fname = strrep(obj.bin_fname, '.lf.bin', '.lf.meta');
            
            % make sure it exists
            assert(isfile(obj.meta_fname), 'associated lf.meta file doesn''t exist')
            ...
            obj.uV_per_bit          = 1e6 * str2double(obj.config.imAiRangeMax) / 512 / 250;
