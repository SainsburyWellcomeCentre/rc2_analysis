...
%    ks4_npy                     - load one of the many .npy files in the Kilosort directory
%    cluster_groups              - load the .csv file containing information about clusters, i.e. whether they are 'good' or 'noise'
%    ks4_label                   - load the cluster_KSLabel.tsv file generated by the SpikeInterface + Kilosort4 pipeline
...
%    metrics_csv                 - load the metrics.csv file generated by the SpikeInterface + Kilosort4 pipeline
%    waveform_metrics_csv        - load the waveform_metrics.csv file generated by the SpikeInterface + Kilosort4 pipeline
%    waveform_metrics_fixed_csv  - load the waveform_metrics_fix.csv file with waveforms
%    clusters_to_check_xlsx       - load the .xlsx file saved after manual inspection of the data
%    mua_clusters_to_check_xlsx   - load the .xlsx file saved after manual inspection of the MUA data
...
%          2. several methods need to cope with the event that a file does exist
    ...
    methods
        ...
        function val = ks4_npy(obj, probe_id, var)
         %%ks4_npy Load one of the many .npy files in the Kilosort output directory
        %
        %   DATA = ks4_npy(PROBE_ID, VARIABLES)
        ...
        %   See XX for more information on those files.
            ...
            fname = obj.file_manager.ks4_npy(probe_id, var);
        ...
        function tbl = ks4_label(obj, probe_id)
        %%ks4_label Load the cluster_KSLabel.tsv file generated by the SpikeInterface + Kilosort4 pipeline
        %
        %   TABLE = ks4_label(PROBE_ID)
        %   loads the .csv for probe recording with ID, PROBE_ID and
        %   returns it as a MATLAB table, TABLE.
        
            fname = obj.file_manager.ks4_label(probe_id);
            tbl = [];
            if isfile(fname)
        ...
        function tbl = clusters_to_check_csv(obj, probe_id)
        %%clusters_to_check_csv Load the editable auto-curation .csv (with the `keep` column)
        %
        %   TABLE = clusters_to_check_csv(PROBE_ID)
        %   loads the automated Bombcell + metric curation .csv (possibly
        %   hand-edited) for probe recording PROBE_ID and returns it as a
        %   MATLAB table, TABLE. The `keep` column drives cluster selection.
        ...
        function tbl = mua_clusters_to_check_csv(obj, probe_id)
        %%mua_clusters_to_check_csv Load the editable auto-curation MUA .csv (with the `keep` column)
        %
        %   TABLE = mua_clusters_to_check_csv(PROBE_ID)
        %   loads the automated MUA curation .csv (possibly hand-edited) for
        %   probe recording PROBE_ID and returns it as a MATLAB table, TABLE.
        ...
        function tbl = clusters_to_check_xlsx(obj, probe_id)
        %%clusters_to_check_xlsx (legacy) Load the manual-curation .xlsx
        %
        %   TABLE = clusters_to_check_xlsx(PROBE_ID)
        %   loads the .xlsx for probe recording with ID, PROBE_ID and
        %   returns it as a MATLAB table, TABLE.
        %   NOTE: superseded by clusters_to_check_csv (automated curation with
        %   an editable `keep` column); kept only for legacy datasets.

            fname = obj.file_manager.clusters_to_check_xlsx(probe_id);
            tbl = readtable(fname);
        end
        
        
        
        function tbl = mua_clusters_to_check_xlsx(obj, probe_id)
        %%mua_clusters_to_check_xlsx Load the .xlsx file saved after manual
        %%inspection of the MUA data 
        %
        %   TABLE = mua_clusters_to_check_xlsx(PROBE_ID)
        %   loads the .xlsx for probe recording with ID, PROBE_ID and
        %   returns it as a MATLAB table, TABLE.
        
            fname = obj.file_manager.mua_clusters_to_check_xlsx(probe_id);
            tbl = readtable(fname);
        end
