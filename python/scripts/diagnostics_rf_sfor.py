...
def compute_partition(run: str, limit: int | None = None) -> tuple[pd.DataFrame, object, object]:
    ...
    return pd.DataFrame(rows), cfg, folder


def _render(df: pd.DataFrame, cfg, folder, run: str) -> None:
    """Render every diagnostic figure from a partition frame into
    ``<run>/diagnostics/`` — shared by ``generate`` (after computing) and
    ``--figures-only`` (after reading the saved CSV), so the figures never drift."""
...
def figures_only(run: str) -> int:
    ...
    if not csv.exists():
        ...
        return 1
    _render(pd.read_csv(csv), cfg_fn(), folder, run)
    return 0
