"""Generalised VIF for the FULL current 20 ms design — all predictors:
Speed / TF / Onset / SF / OR / ME_face / History.

History depends on the cluster's own spikes (convolved), so unlike the
stimulus-only VIF this is computed per cohort cluster and medianed. ME_face is
the full per-bin block (not the across/within split).

Output: current_plus_ME_20ms/diagnostics/collinearity_vif_full.{pdf,png}
"""
...
from scripts.collinearity_vif_me import _gvif   # log-det stable GVIF

PROBES = ("CAA-1123243_rec1", "CAA-1123244_rec1", "CAA-1123466_rec1")
OUT = drv.ROOT / "figures" / "glm" / "current_plus_ME_20ms" / "diagnostics"
BLOCKS = ["Speed", "TF", "Onset", "SF", "OR", "ME_face", "History"]
...
def main() -> int:
    ...
    print("\n=== full 20 ms design GVIF (median over cohort) ===")
    ...
    ax.text(len(BLOCKS) - 0.5, GOOD + 0.02, "worrying", fontsize=7.5, color="0.4")
    ax.text(len(BLOCKS) - 0.5, WORRY + 0.02, "severe", fontsize=7.5, color="0.4")
    ax.set_ylabel("generalised VIF (SE scale, GVIF$^{1/2df}$)")
    ax.set_ylim(0, max(WORRY + 0.3, max(vals) * 1.15))
    ax.set_title("Full 20 ms design — collinearity per block "
                 f"(median over {len(df)} cohort clusters)",
